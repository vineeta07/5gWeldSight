...
const TOGGLES = [
  ...
  { label: "Use demo data when offline", key: "demoSim", defaultVal: true },
  { label: "Auto-acknowledge low-risk reports", key: "autoAck", defaultVal: false },
  { label: "Show detection overlays", key: "aiOverlays", defaultVal: true },
  { label: "Show weld zones", key: "zoneOverlays", defaultVal: true },
];
  ...
  return (
    <div className="p-4 space-y-4 max-w-3xl">
      ...
      <div className="glass-panel p-4">
        <div className="font-mono text-xs text-cyan-400 mb-4">Your account</div>
      ...
      {/* Platform config */}
      <div className="glass-panel p-4">
        <div className="font-mono text-xs text-cyan-600 font-medium mb-4">Display</div>
      ...
      <div className="glass-panel p-4">
        <div className="font-mono text-xs text-cyan-600 font-medium mb-4">Record verification (optional)</div>
        <div className="space-y-3">
          <div className="flex flex-col py-2 border-b border-slate-200">
            <div className="flex items-center justify-between mb-2">
              <span className="font-mono text-xs text-slate-600">Pending approvals (demo)</span>
          ...
          <div className="flex items-center justify-between py-2 border-b border-slate-200">
            <span className="font-mono text-xs text-slate-600">Connection</span>
            <span className="font-mono text-xs text-green-500 font-bold">Ledger not connected (optional)</span>
          </div>
        </div>
      </div>

      <div className="flex items-center gap-3">
        <button onClick={save} className="px-6 py-2 font-mono text-sm transition-all rounded text-white" style={{ background: "#10b981" }}>
          {saved ? "✓ SAVED" : "SAVE SETTINGS"}
        </button>
        <div className="font-mono text-xs text-slate-400">WeldSight · Delhi Technological University</div>
      </div>
    </div>
  );
}
