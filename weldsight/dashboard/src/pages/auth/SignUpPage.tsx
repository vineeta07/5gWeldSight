  ...
  if (success) return (
    <AuthLayout>
      <div className="animate-fade-in text-center space-y-4">
        ...
        <Link to="/login" className="block mt-4 text-blue-600 font-mono text-sm hover:text-blue-500 font-bold">← Back to Sign In</Link>
      </div>
    </AuthLayout>
  );
    ...
    <AuthLayout>
      <div className="animate-fade-in">
        <div className="mb-8">
          ...
          <h1 className="font-bold text-3xl text-slate-900 tracking-tight mb-2" style={{ fontFamily: "Inter, system-ui, sans-serif" }}>Create an account</h1>
          <p className="text-slate-500 text-sm">You will get an email to confirm your address.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {[
            ...
            { label: "EMAIL ADDRESS", type: "email", value: email, set: setEmail, placeholder: "inspector@weldsight.com" },
            ...
            <div key={label}>
              <label className="block text-xs font-mono text-slate-600 mb-1.5 font-bold">{label}</label>
              <input
                type={type}
                value={value}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-2.5 font-bold text-sm disabled:opacity-50 transition-all text-white rounded hover:shadow-lg"
        ...
        <div className="mt-6 text-center">
          ...
          <Link to="/login" className="text-blue-600 text-xs font-mono hover:text-blue-500 transition-colors font-bold">Sign In</Link>
        </div>
      </div>
    </AuthLayout>
  );
