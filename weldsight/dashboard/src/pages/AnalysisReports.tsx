      ...
      try {
        ...
        if (selectedReport === "summary") {
          ...
          if (inc) {
            ...
            doc.setFontSize(12);
            doc.setFont("helvetica", "bold");
            doc.text("Why it was flagged", 14, y);
            y += 8;
            doc.setFontSize(9);
            doc.setFont("helvetica", "normal");
        ...
        doc.text("WELDSIGHT — 5G AI WELD INSPECTION PLATFORM | CONFIDENTIAL", 14, pageH - 6);

        doc.save(`WELDSIGHT_Report_${selectedReport === "summary" ? "Overview" : selectedReport}_${new Date().toISOString().slice(0, 10)}.pdf`);
      } catch (e) {
    ...
    setTimeout(() => {
      try {
        ...
        if (selectedReport === "summary") {
          ...
          const statBoxes = [
            { label: "Total\nfindings", value: `${totalAnomalies}`, bg: "F1F5F9", color: "0F172A" },
            { label: "High\nrisk", value: `${highSeverity}`, bg: "FEF2F2", color: "DC2626" },
            { label: "Medium\nrisk", value: `${mediumSeverity}`, bg: "FFFBEB", color: "D97706" },
          ...
          if (inc) {
            ...
            // Slide 3: Why it was flagged
            const slide3 = pptx.addSlide();
            slide3.background = { color: "FFFFFF" };
            slide3.addText("Why it was flagged", { x: 0.5, y: 0.3, w: 9, fontSize: 22, color: "0F172A", bold: true });
        ...
        pptx.writeFile({ fileName: `WELDSIGHT_Report_${selectedReport === "summary" ? "Overview" : selectedReport}_${new Date().toISOString().slice(0, 10)}.pptx` });
      } catch (e) {
  ...
  return (
    <div className="p-6 space-y-6 max-w-6xl mx-auto h-full flex flex-col">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-display font-bold text-2xl text-slate-900 tracking-wide">Reports</h1>
        </div>
              ...
              <section>
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                  <div className="bg-slate-50 border border-slate-200 p-4 rounded-lg">
                    <div className="text-xs font-mono text-slate-500 mb-1">Total findings</div>
                    ...
                    <div key={alert.id} className={`border rounded-lg p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${alert.severity === 'CRITICAL' || alert.severity === 'HIGH' ? 'bg-red-50 border-red-100' : 'bg-slate-50 border-slate-200'}`}>
                      <div>
                        <div className="flex items-center gap-3 mb-1">
                          ...
                          <span className={`px-2 py-0.5 text-[10px] font-bold rounded ${alert.severity === 'CRITICAL' ? 'bg-red-600 text-white' : alert.severity === 'HIGH' ? 'bg-red-200 text-red-800' : alert.severity === 'MEDIUM' ? 'bg-amber-200 text-amber-800' : 'bg-green-200 text-green-800'}`}>
                            {alert.severity}
                          </span>
                        </div>
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    <div className="mt-6">
                      <h4 className="font-bold text-slate-800 mb-3 border-b border-slate-100 pb-2">Why it was flagged</h4>
