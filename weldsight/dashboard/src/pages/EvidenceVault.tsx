...
import { CHAIN_GATEWAY_URL } from "../lib/api";
const CHAIN_GATEWAY = CHAIN_GATEWAY_URL || "http://localhost:3001"; // optional ledger service (extras/chain-gateway)
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="font-display font-bold text-2xl text-slate-900 tracking-wide">Inspection records</h1>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          {["ALL", "CRITICAL", "HIGH", "MEDIUM", "LOW"].map(r => (
            <button key={r} onClick={() => setFilterRisk(r)} className={`px-2 py-0.5 font-mono text-xs border rounded transition-colors ${filterRisk === r ? "border-emerald-500 text-emerald-600 bg-emerald-50" : "border-slate-300 text-slate-500 hover:border-slate-400"}`}>{r === "ALL" ? "All" : r.charAt(0) + r.slice(1).toLowerCase()}</button>
          ))}
          <div className="w-px h-4 bg-slate-300" />
          {["ALL", "VERIFIED", "PENDING", "FLAGGED"].map(s => (
            <button key={s} onClick={() => setFilterStatus(s)} className={`px-2 py-0.5 font-mono text-xs border rounded transition-colors ${filterStatus === s ? "border-emerald-500 text-emerald-600 bg-emerald-50" : "border-slate-300 text-slate-500 hover:border-slate-400"}`}>{s === "ALL" ? "All" : s.charAt(0) + s.slice(1).toLowerCase()}</button>
          ))}
        </div>
      </div>
            ...
            <div className="relative h-32 overflow-hidden rounded bg-slate-100">
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center">
                  ...
                  <div className="font-mono text-[10px] text-slate-400 mt-1">Evidence image</div>
                </div>
              </div>
              {/* Corner brackets */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => { setVerifyModal(null); setVerifyState("idle"); }}>
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 p-5 space-y-4 animate-fade-in" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <div>
                <div className="font-mono text-xs text-emerald-700 font-bold">Ledger check</div>
            ...
            <div className="border-t border-slate-200 pt-3">
              <div className="font-mono text-[10px] text-slate-500 mb-1">Local fingerprint</div>
            ...
            {verifyState === "loading" && (
              <div className="text-center py-4">
                <div className="font-mono text-xs text-emerald-600 animate-pulse font-bold">Checking the ledger…</div>
                <div className="font-mono text-[10px] text-slate-400 mt-1">Contacting the chain gateway</div>
              </div>
            )}

            {verifyState === "anchoring" && (
              <div className="text-center py-4">
                <div className="font-mono text-xs text-amber-600 animate-pulse font-bold">Saving to the ledger…</div>
                <div className="font-mono text-[10px] text-slate-400 mt-1">Submitting…</div>
              </div>
            )}
              ...
              <div className="space-y-3">
                {verifyResult.hashMatch !== undefined ? (
                  ...
                  <div className="p-3 rounded border bg-emerald-50 border-emerald-200">
                    <div className="font-mono text-sm font-bold text-emerald-700">Saved on the ledger</div>
            ...
            {verifyState === "idle" && verifyResult?.notAnchored && (
              <div className="space-y-3">
                <div className="p-3 rounded border bg-amber-50 border-amber-200">
                  <div className="font-mono text-sm font-bold text-amber-700">Not on the ledger yet</div>
                  <div className="font-mono text-[10px] text-slate-500 mt-1">This record hasn't been saved to the ledger yet.</div>
                </div>
                <button
            ...
            {verifyState === "error" && (
              <div className="p-3 rounded border bg-red-50 border-red-200">
                <div className="font-mono text-sm font-bold text-red-700">Couldn't reach the chain gateway (optional service)</div>
